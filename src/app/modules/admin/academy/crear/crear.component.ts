...
export class CrearProyectoComponent implements OnInit {
    ...
    selectedFilesMobil?: FileList;
    currentFileMobil?: File;
    progressMobil = 0;
    messageMobil = '';
    previewMobil = '';
    urlImagenMobil = '';
    ...
    constructor(
        ...
        this._activatedRoute.paramMap.subscribe((params: ParamMap) => {
            this.urlAnterior = params.get('id').split('/')[0];
            this.idEdit = params.get('id').split('/')[1];
    ...
    cargarDatos() {
        this._activosService.findOneById(this.idEdit).subscribe((data: any) => {
            ...
            this.crearFormGroup
                ...
                .setValue(data.personResponsible);
            this.crearFormGroup.get('status').setValue(data.status);
            this.urlImagen = data.picture;
            this.preview = data.picture;
            this.crearFormGroup.get('fechaInicio').setValue(data.startDate);
            const client = data.client[0].ruc
            this.crearFormGroup.get('ruc').setValue(client)
            this.clientRuc = client
        });
    }

    onClickGuardar(value: any) {
        console.log('data para actualizar ',value);
        if (this.crearFormGroup.valid) {
            console.log('listo para actualizar', this.clientRuc)
            let insert: any = {
                name: value.titulo,
                active: value.activo,
                type: value.tiempo,
                ...
                startDate: this.datePipe
                    ...
                    .toString(),
                imageUrlDesktop: this.urlImagen ? this.urlImagen : 'default',
                createdUserId: this.idUsuario,
                targetUrl: '',
                ruc: this.clientRuc //this.clientRuc.ruc.name + ' ' +this.clientRuc.ruc.lastName,
            };
            ...
            if (insert.name === '') {
                ...
                // if (insert.name === '') {
                    ...
                    Object.keys(this.crearProgramaFormGroup.controls).forEach(
                        (key) => {
                            this.crearProgramaFormGroup.controls[
                                key
                            ].markAsTouched();
                        }
                    );
                ...
                if (this.editar) {
                    ...
                    this._activosService
                        .updateOneById(insert)
                        .subscribe((data) => {
                            this.alertService.mensajeSuccess(
                                'Actualizado Correctamente'
                            );
                            this.routerService.navigateByUrl('/academy/' + this.idUsuario);
                        });
                } else {
    ...
    anterior() {
        this.routerService.navigateByUrl('/academy/'+this.urlAnterior);
    }
